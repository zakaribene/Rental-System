  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-ink-100 bg-white/80 px-4 backdrop-blur dark:border-ink-800 dark:bg-ink-900/80 sm:px-6 lg:px-8">
      <div className="flex min-w-0 items-center gap-3">
        <button
          onClick={onMenuClick}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-ink-500 transition-colors hover:bg-ink-100 dark:text-white dark:hover:bg-ink-800 md:hidden"
        >
          <Menu size={20} />
        </button>
      ...
      <div className="flex shrink-0 items-center gap-1 sm:gap-2">
        <button
          onClick={toggleTheme}
          className="flex h-10 w-10 items-center justify-center rounded-full text-ink-500 transition-colors hover:bg-ink-100 dark:text-white dark:hover:bg-ink-800"
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            ...
            <div className="hidden text-left leading-tight sm:block">
              <p className="text-sm font-semibold text-ink-800 dark:text-white">{user?.name || 'Super Admin'}</p>
              <p className="text-xs text-ink-400 dark:text-ink-300">{user?.phone || '—'}</p>
            </div>
            <ChevronDown size={16} className="hidden text-ink-400 dark:text-ink-200 sm:block" />
          </button>
