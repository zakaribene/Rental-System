...
const getMyStore = async (req, res, next) => {
  try {
    const store = await Store.findById(req.storeId).select(
      "storeName ownerName status salesEnabled expensesEnabled logoUrl createdAt subscriptionStatus subscriptionEndsAt gracePeriodEndsAt graceDays graceHours graceMinutes graceBannerColor graceMessage"
    );
